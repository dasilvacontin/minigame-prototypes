...
function updateGamepads(gameState) {
    if (!isWindowFocused) return; // Skip gamepad updates if window is not focused
    ...
    if (isOnline) {
        ...
        if (localPlayer) {
            // Combine all gamepad inputs
            const combinedGamepadState = gameState.gamepads.reduce((acc, gp) => {
                acc.up = acc.up || gp.up;
                acc.down = acc.down || gp.down;
                acc.left = acc.left || gp.left;
                acc.right = acc.right || gp.right;
                acc.A = acc.A || gp.A;
                acc.B = acc.B || gp.B;
                return acc;
            }, { up: false, down: false, left: false, right: false, A: false, B: false });

            localPlayer.gamepad = combinedGamepadState; // Use combined gamepad state
        }
    } else {
        // Function to get a unique color
        function getUniqueColor() {
            // Collect used colors from current players
            const usedColors = new Set(gameState.players.map(player => player.color));
        ...
        gameState.gamepads.forEach((gp, index) => {
            ...
            if (!player) {
                player = createPlayer(
                    getUniqueColor(), // Use the unique color function
                    index
                );
